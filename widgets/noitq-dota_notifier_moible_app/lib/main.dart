...
void main() {
  ...
  runApp(MultiBlocProvider(
    providers: <BlocProvider>[
      ...
      BlocProvider<ProPlayersBloc>(
        create: (_) => ProPlayersBloc()..add(ProPlayersFetchStarted()),
      ),
      BlocProvider<SubscribesBloc>(create: (_) => SubscribesBloc()),
      BlocProvider<FirebaseBloc>(
        create: (_) => FirebaseBloc()..add(FirebaseAppStarted()),
      ),
      BlocProvider<ProfileBloc>(create: (_) => ProfileBloc()),
      BlocProvider<LocalNotificationBloc>(
        create: (_) => LocalNotificationBloc()..add(LnAppStarted()),
      ),
    ],
    ...
    return GestureDetector(
      ...
      child: MaterialApp(
        ...
        theme: new ThemeData(
          ...
          iconTheme: IconThemeData(color: Color(0xff008ff4)),
        ...
        home: BlocBuilder<AuthBloc, AuthState>(
          builder: (context1, state) {
            if (state is AuthAuthenticated) {
              BlocProvider.of<FirebaseBloc>(context).localNotificationBloc =
                  BlocProvider.of<LocalNotificationBloc>(context);

              BlocProvider.of<SubscribesBloc>(context).authBloc =
                  BlocProvider.of<AuthBloc>(context);
              BlocProvider.of<SubscribesBloc>(context).firebaseBloc =
                  BlocProvider.of<FirebaseBloc>(context);
              BlocProvider.of<SubscribesBloc>(context).add(
                SubscribesFetchStarted(),
              );

              BlocProvider.of<ProfileBloc>(context).authBloc =
                  BlocProvider.of<AuthBloc>(context);
              BlocProvider.of<ProfileBloc>(context).firebaseBloc =
                  BlocProvider.of<FirebaseBloc>(context);

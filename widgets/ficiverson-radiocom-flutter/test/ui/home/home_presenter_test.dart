...
void main() {
  ...
  setUpAll(() async {
    ...
    Injector.appInstance.registerDependency<HomeView>(
      () => view,
      override: true,
    );
    Injector.appInstance.registerDependency<HomeRouterContract>(
      () => router,
      override: true,
    );
  ...
  test('that can init the presenter and load all data', () async {
    when(mockRepository.getLiveBroadcast()).thenAnswer(
      (_) => MockRadiocoRepository.now(),
    );
    when(mockRepository.getTimetableData(any, any)).thenAnswer(
      (_) => MockRadiocoRepository.timetables(),
    );
    when(mockRepository.getEpisodes(any)).thenAnswer(
      (_) => MockRadiocoRepository.episodes(),
    );
    when(mockRepository.getAllPodcasts()).thenAnswer(
      (_) => MockRadiocoRepository.podcasts(),
    );
    when(mockRepository.getRadioStationData()).thenAnswer(
      (_) => MockRadiocoRepository.radioStation(),
    );
    when(mockRepository.getNews()).thenAnswer(
      (_) => MockRadiocoRepository.news(),
    );
    when(mockConnection.isConnectionAvailable()).thenAnswer(
      (_) => Future.value(true),
    );
  ...
  test('that can init the presenter without connection', () async {
    when(mockRepository.getLiveBroadcast()).thenAnswer(
      (_) => MockRadiocoRepository.now(),
    );
    when(mockRepository.getTimetableData(any, any)).thenAnswer(
  ...
  test('that can init the presenter withhout service working', () async {
    when(mockRepository.getLiveBroadcast()).thenAnswer(
      (_) => MockRadiocoRepository.now(isEmpty: true),
    );
    when(mockRepository.getTimetableData(any, any)).thenAnswer(
      (_) => MockRadiocoRepository.timetables(isEmpty: true),
    );
    when(mockRepository.getEpisodes(any)).thenAnswer(
      (_) => MockRadiocoRepository.episodes(isEmpty: true),
    );
    when(mockRepository.getAllPodcasts()).thenAnswer(
      (_) => MockRadiocoRepository.podcasts(isEmpty: true),
    );
    when(mockRepository.getRadioStationData()).thenAnswer(
      (_) => MockRadiocoRepository.radioStation(isEmpty: true),
    );
    when(mockRepository.getNews()).thenAnswer(
      (_) => MockRadiocoRepository.news(isEmpty: true),
    );
    when(mockConnection.isConnectionAvailable()).thenAnswer(
      (_) => Future.value(true),
    );
  ...
  test('that can resume the view and realod the data', () async {
    when(mockRepository.getLiveBroadcast()).thenAnswer(
      (_) => MockRadiocoRepository.now(),
    );
    when(mockRepository.getTimetableData(any, any)).thenAnswer(
  ...
  test(
    'that can resume the view and realod the data with error response reload the view with base now',
    () async {
      when(mockRepository.getLiveBroadcast()).thenAnswer(
        (_) => MockRadiocoRepository.now(isEmpty: true),
      );
  ...
  test('that can navigate to podcast controls', () async {
    ...
    expect(
      (router.data[0] as Episode).title,
      equals(EpisodeInstrument.givenAnEpisode().title),
    );
  });

  test('that can navigate to all podcast', () async {
    when(mockRepository.getLiveBroadcast()).thenAnswer(
      (_) => MockRadiocoRepository.now(),
    );
    when(mockRepository.getTimetableData(any, any)).thenAnswer(
  ...
  test('that can navigate to all podcast with category', () async {
    when(mockRepository.getLiveBroadcast()).thenAnswer(
      (_) => MockRadiocoRepository.now(),
    );
    when(mockRepository.getTimetableData(any, any)).thenAnswer(
  ...
  test('that can navigate to now playing', () async {
    when(mockRepository.getLiveBroadcast()).thenAnswer(
      (_) => MockRadiocoRepository.now(),
    );
    when(mockRepository.getTimetableData(any, any)).thenAnswer(
  ...
  test('that can navigate to new detail', () async {
    when(mockRepository.getLiveBroadcast()).thenAnswer(
      (_) => MockRadiocoRepository.now(),
    );
    when(mockRepository.getTimetableData(any, any)).thenAnswer(
    ...
    expect(
      (router.data[0] as New).title,
      equals(NewInstrument.givenANew().title),
    );
  });
  ...
  test('that can navigate to podcast detail', () async {
    ...
    expect(
      (router.data[0] as Program).name,
      equals(ProgramInstrument.givenAProgram().name),
    );
  });

  test('that can play podcast in stop state', () async {
    when(mockRepository.getLiveBroadcast()).thenAnswer(
      (_) => MockRadiocoRepository.now(),
    );
    when(mockRepository.getTimetableData(any, any)).thenAnswer(
  ...
  test('that can resume podcast', () async {
    when(mockRepository.getLiveBroadcast()).thenAnswer(
      (_) => MockRadiocoRepository.now(),
    );
    when(mockRepository.getTimetableData(any, any)).thenAnswer(
  ...
  test('that can pause streaming', () async {
    when(mockRepository.getLiveBroadcast()).thenAnswer(
      (_) => MockRadiocoRepository.now(),
    );
    when(mockRepository.getTimetableData(any, any)).thenAnswer(
    ...
    when(mockPlayer.isPodcast).thenReturn(false);
    when(mockPlayer.stop()).thenAnswer((_) => Future.value(true));
  ...
  test('that can pause podcast', () async {
    when(mockRepository.getLiveBroadcast()).thenAnswer(
      (_) => MockRadiocoRepository.now(),
    );
    when(mockRepository.getTimetableData(any, any)).thenAnswer(
    ...
    when(mockPlayer.isPodcast).thenReturn(true);
    when(mockPlayer.pause()).thenAnswer((_) => Future.value(true));
  ...
  test('that can play streaming when its not playing', () async {
    when(mockRepository.getLiveBroadcast()).thenAnswer(
      (_) => MockRadiocoRepository.now(),
    );
    when(mockRepository.getTimetableData(any, any)).thenAnswer(
  ...
  test('that can play streaming when its playing', () async {
    when(mockRepository.getLiveBroadcast()).thenAnswer(
      (_) => MockRadiocoRepository.now(),
    );
    when(mockRepository.getTimetableData(any, any)).thenAnswer(
  ...
  test('that cannot play streaming when its not playing', () async {
    when(mockRepository.getLiveBroadcast()).thenAnswer(
      (_) => MockRadiocoRepository.now(),
    );
    when(mockRepository.getTimetableData(any, any)).thenAnswer(
  ...
  test('that cannot play streaming when its playing', () async {
    when(mockRepository.getLiveBroadcast()).thenAnswer(
      (_) => MockRadiocoRepository.now(),
    );
    when(mockRepository.getTimetableData(any, any)).thenAnswer(

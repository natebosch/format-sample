...
Future<List<Episode>> getDownloads() async {
  ...
  return Future.wait(
    episodes.map((UserEpisode episode) => getEpisodeWithActions(episode)),
  );
}
...
Future<Episode> getEpisodeWithActions(UserEpisode userEpisode) async {
  ...
  final EpisodeAction downloadAction = dash.find(
    episodeActions,
    (EpisodeAction action) =>
        action.type == EpisodeActionType.DOWNLOAD.toString(),
  );
  final EpisodeAction favoriteAction = dash.find(
    episodeActions,
    (EpisodeAction action) =>
        action.type == EpisodeActionType.FAVORITE.toString(),
  );
  final EpisodeAction finishAction = dash.find(
    episodeActions,
    (EpisodeAction action) =>
        action.type == EpisodeActionType.FINISH.toString(),
  );
  final EpisodeAction playAction = dash.find(
    episodeActions,
    (EpisodeAction action) => action.type == EpisodeActionType.PLAY.toString(),
  );
...
Future<Episode> getUserEpisodeFromUrl(String url) async {
  final App app = App();
  final UserEpisodeBean userEpisodeModel = app.models['user_episode'];
  final UserEpisode userEpisode =
      await userEpisodeModel.findOneWhere(userEpisodeModel.url.eq(url));
...
Future<void> downloadEpisode(
  ...
  final String downloadPath = join(
    await app.getApplicationDownloadsPath(),
    '$downloadId.mp3',
  );
  ...
  await userEpisodeModel.insert(
    UserEpisode(details: episode.toJson(), url: episode.url),
  );
  await episodeActionModel.removeWhere(episodeActionModel.url.eq(episode.url));
...
Future<void> unfavoriteEpisode(Episode episode) async {
  final App app = App();
  final EpisodeActionBean episodeActionModel = app.models['episode_action'];
  await episodeActionModel.removeWhere(episodeActionModel.url
      .eq(episode.url)
...
Future<void> deleteEpisode(Episode episode) async {
  final App app = App();
  final EpisodeActionBean episodeActionModel = app.models['episode_action'];
  final EpisodeAction download = await episodeActionModel.findOneWhere(
    episodeActionModel.url.eq(episode.url),
  );

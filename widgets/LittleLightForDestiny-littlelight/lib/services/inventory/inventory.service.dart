...
class TransferDestination {
  ...
  TransferDestination(
    this.type, {
    this.action = InventoryAction.Transfer,
    this.characterId,
  });
}
...
class InventoryService {
  ...
  equip(
    ...
    try {
      ...
      _broadcaster.push(NotificationEvent(
        NotificationType.requestedEquip,
        item: item,
        characterId: destinationCharacterId,
      ));
  ...
  equipMultiple(
    ...
    profile.pauseAutomaticUpdater = true;
    DestinyCharacterComponent character = profile.getCharacter(
    ...
    List<ItemWithOwner> itemsToTransfer = itemStates.where((i) {
      ...
      if (def?.nonTransferrable == true && i?.ownerId != character.characterId)
        return false;
      if (![character?.classType, DestinyClass.Unknown].contains(
        def?.classType,
      )) return false;
      return true;
    }).toList();
    ...
    await _equipMultiple(
      itemsToEquip.map((i) => i.item).toList(),
      destinationCharacterId,
    );
  ...
  transferLoadout(
    ...
    List<DestinyItemComponent> items = profile.getItemsByInstanceId(
      equippedIds + unequippedIds,
    );
  ...
  Future<dynamic> _transfer(
    ...
    if (onPostmaster) {
      ...
      if (result != 0) {
        throw TransferError(
          TransferErrorCode.cantPullFromPostmaster,
          item,
          destination,
          sourceCharacterId,
        );
    ...
    if (needsVaulting) {
      int result = await api.transferItem(
        item.itemHash,
        stackSize,
        true,
        item.itemInstanceId,
  ...
  _equip(
    ...
    DestinyItemComponent currentlyEquipped = equipment.firstWhere(
    ...
    List<DestinyItemComponent> inventory = profile.getCharacterInventory(
      characterId,
    );
    ...
    DestinyItemInstanceComponent iInfo = profile.getInstanceInfo(
      item.itemInstanceId,
    );
    DestinyItemInstanceComponent sInfo = profile.getInstanceInfo(
      currentlyEquipped.itemInstanceId,
    );
  ...
  _equipMultiple(
    ...
    List<DestinyItemComponent> charEquipment = profile.getCharacterEquipment(
      characterId,
    );
    bucketHashes.forEach((bucketHash) {
      previouslyEquipped[bucketHash] = charEquipment.firstWhere(
    ...
    List<DestinyItemComponent> charInventory = profile.getCharacterInventory(
      characterId,
    );
    result.forEach((result) {
      ...
      charEquipment.removeWhere(
        (item) => item.itemInstanceId == previouslyEquippedItem.itemInstanceId,
      );
      charInventory.add(previouslyEquippedItem);

      newlyEquippedInstance.isEquipped = true;
      charInventory.removeWhere(
        (item) => item.itemInstanceId == result.itemInstanceId,
      );
  ...
  _freeSlotsOnBucket(
    ...
    List<DestinyItemComponent> items = _getItemsOnBucket(
      bucketDefinition,
      characterId,
    );
    ...
    await profile.fetchProfileData(
      components: ProfileComponentGroups.inventories,
      skipUpdate: true,
    );
    ...
    items.sort((itemA, itemB) {
      DestinyItemInstanceComponent instA = profile.getInstanceInfo(
        itemA.itemInstanceId,
      );
      DestinyItemInstanceComponent instB = profile.getInstanceInfo(
        itemB.itemInstanceId,
      );
    ...
    for (var i = 0; i < itemsToRemove; i++) {
      try {
        _broadcaster.push(NotificationEvent(
          NotificationType.requestedVaulting,
          item: items[i],
          characterId: characterId,
        ));
  ...
  Future<DestinyItemComponent> _findBlockingExotic(
    ...
    if (![DestinyItemType.Armor, DestinyItemType.Weapon].contains(
      def.itemType,
    )) {
      return null;
    }
  ...
  Future<DestinyItemComponent> _findSubstitute(
    ...
    var itemDef = await manifest.getDefinition<DestinyInventoryItemDefinition>(
      item.itemHash,
    );

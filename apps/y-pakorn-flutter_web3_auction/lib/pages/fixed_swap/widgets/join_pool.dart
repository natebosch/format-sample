...
class JoinPoolPlaceholder extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return [
          ...
          [placeholderBox(80)].row,
          Niku().height(20),
          [placeholderBox(370, 40)].row,
          Spacer(),
          CreateListing(
            textToDisplay: 'Buy',
            color: kGrey,
    ...
    return [
          ...
          StreamBuilder<BigInt>(
            ...
            builder: (_, snapshot) {
              ...
              return [
                '${toDecimal(value).toStringAsFixed(2)}/${toDecimal(
                      p.pool!.amountTotalToken1,
                    ).toStringAsFixed(2)}'
          ...
          Expanded(
            child: 'Input Amount Desired in ${p.pool!.currency.symbol}'
                ...
                .cursorColor(kBlack)
                .errorText(
                  p.amountController.tokenIsError
                      ? p.amountController.errorText
                      : null,
                )
                .suffix('100%'.text(TypeWeight.bold).scale().onTap(() {
                  p.amountController.focusNode.unfocus();
                  p.setAmountMax();
                }).niku().padding(EdgeInsets.only(left: 10))),
          ),
          //Spacer(),
          CreateListing(
            textToDisplay: 'Buy',
            ...
            p.isLoading || p.amountController.text.isEmpty
                ? null
                : () async {
                    //() async {
                    p.amountController.focusNode.unfocus();

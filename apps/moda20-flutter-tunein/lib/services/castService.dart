...
enum CastState { CASTING, NOT_CASTING }
...
class CastService {
          ...
          positionTimer = Timer.periodic(Duration(seconds: 1), (Timer) async {
            if (castingState.value == CastState.CASTING &&
              ...
              UpnPPlugin.getPositionInfo(
                ...
                Duration positionInDuration = FormatPositionToDuration(
                  position,
                );
                if (positionInDuration != null &&
  ...
  Future castAndPlay(
    ...
    if (_castingState.value == CastState.CASTING || SingleCast) {
      ...
      UpnPPlugin.setCurrentURI(
        service: await currentDev.getService(
  ...
  _initStreams() {
    ...
    _currentPosition = BehaviorSubject<Duration>.seeded(
      Duration(milliseconds: 0),
    );
    _castingPlayerState = BehaviorSubject<PlayerState>.seeded(
      PlayerState.stopped,
    );

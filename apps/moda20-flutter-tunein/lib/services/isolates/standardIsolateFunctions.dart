...
class StandardIsolateFunctions {
  ...
  static Future<List<Artist>> fetchArtistsFromAlbums(List<Album> albums) async {
    ...
    ItemsList.forEach((Album album) {
      if (artists["${album.artist}"] != null) {
        ...
        artists["${album.artist}"] = new Artist(
          currentIndex,
          album.artist,
          null,
          null,
  ...
  static Future<List<Album>> fetchAlbumFromsongs(
    List<Tune> songs, {
    Function(List<Album>) callback,
  }) async {
    ...
    songs.forEach((Tune tune) {
      if (albums["${tune.album}${tune.artist}"] != null) {
        ...
        albums["${tune.album}${tune.artist}"] = new Album(
          currentIndex,
          tune.album,
          tune.artist,
          tune.albumArt,
  ...
  static Map getTopAlbum(
    Map<String, dynamic> GlobalSongPlayTime,
    ...
    List<Album> topAlbums = newSongMap.keys.map((e) {
      ...
      Album albumFound = albums.values.firstWhere(
        (element) => (element.title == e.album && element.artist == e.artist),
      );
      if (newSongMap[e] != 0) {
    ...
    topAlbums.removeWhere(
      (element) => element == null || element.title == null,
    );
    ...
    if (newValue.length != 0) {
      ...
      if (newSongMap.length < 10) {
        ...
        for (int i = 0; i < 10 - newSongMap.length; i++) {
          ...
          int albumIndex = MathUtils.getRandomFromRange(
            0,
            artistToPickFrom.albums.length,
          );
  ...
  static List<Artist> _getDiscoverableArtistsFromTpArtists(
    ...
    for (int i = 0; i < 4; i++) {
      randomIndexList.add(
        MathUtils.getRandomFromRange(0, notTopArtists.length),
      );
    }

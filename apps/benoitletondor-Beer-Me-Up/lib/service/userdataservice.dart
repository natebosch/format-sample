...
abstract class UserDataService {
  static final UserDataService instance = _UserDataServiceImpl(
    Firestore.instance,
    HttpClient(),
    UntappdService(BeerMeUpApp.config),
  );
...
class _UserDataServiceImpl implements UserDataService {
  ...
  _UserDataServiceImpl(
    this._firestore,
    this._httpClient,
    this._beerSearchService,
  );
  ...
  Future<CheckinFetchResponse> fetchCheckInHistory({CheckIn startAfter}) async {
    ...
    return CheckinFetchResponse(
      checkIns,
      checkIns.length >= _NUMBER_OF_RESULTS_FOR_HISTORY ? true : false,
    );
  }
  ...
  Future<List<BeerCheckInsData>> fetchBeerCheckInsData() async {
    ...
    return beerDocsSnapshot.documents
        .map(
          (beerSnapshot) => BeerCheckInsData(
            _parseBeerFromValue(
              beerSnapshot.data["beer"],
              beerSnapshot.data["beer_version"],
            ),
  ...
  Future<void> saveBeerCheckIn(CheckIn checkIn) async {
    ...
    final numberOfCheckIns =
        beerDocumentValues != null && beerDocumentValues.exists
            ? beerDocumentValues.data["checkin_counter"]
            : 0;
    final drankQuantity =
        beerDocumentValues != null && beerDocumentValues.exists
            ? beerDocumentValues.data["drank_quantity"]
            : 0.0;
    final lastCheckinDate =
        beerDocumentValues != null && beerDocumentValues.exists
            ? beerDocumentValues.data["last_checkin"].toDate()
            : null;

    final lastCheckin = lastCheckinDate != null
        ? checkIn.date.isAfter(lastCheckinDate)
            ? checkIn.date
            : lastCheckinDate
        : checkIn.date;
    ...
    await beerDocument.collection("history").add({
      "date": Timestamp.fromDate(checkIn.date),
      "quantity": checkIn.quantity.value,
    });

    await _userDoc.reference.collection("history").add({
    ...
    int currentPointsCounter = _userDoc.data.containsKey("points")
        ? _userDoc.data["points"] as int
        : 0;

    await _userDoc.reference.setData({
  ...
  Beer _parseBeerFromValue(Map<dynamic, dynamic> data, int version) {
    ...
    if (styleData != null) {
      style = BeerStyle(id: styleData["id"], name: styleData["name"]);
    }
  ...
  Map<String, dynamic> _createValueForBeer(Beer beer) {
    ...
    if (beer.style != null) {
      style = {"id": beer.style.id, "name": beer.style.name};
    }
  ...
  Future<List<CheckIn>> fetchWeekCheckIns(DateTime date) async {
    ...
    final QuerySnapshot snapshots = await _userDoc.reference
        .collection("history")
  ...
  Future<List<Beer>> fetchLastCheckedInBeers() async {
    final querySnapshot = await _userDoc.reference
        .collection("beers")
  ...
  Future<int> fetchRatingForBeer(Beer beer) async {
    final documentSnapshot =
        await _userDoc.reference.collection("beers").document(beer.id).get();

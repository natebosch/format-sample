...
class AppearanceState with _$AppearanceState {
  const factory AppearanceState.initial(AppearanceInfo info) =
      AppearanceStateInitial;

  const factory AppearanceState.themeChanged(AppearanceInfo info) =
      AppearanceStateThemeChanged;

  const factory AppearanceState.trackingNotifyChanged(AppearanceInfo info) =
  ...
  const factory AppearanceState.trayIconChanged(AppearanceInfo info) =
...
class AppearanceSettingsCubit extends Cubit<AppearanceState> {
  ...
  void setTheme(AppThemeType theme) {
    ...
    emit(AppearanceState.themeChanged(state.info.copyWith(theme: theme)));
  }
  ...
  void setLocale(AppLocaleType locale) {
    ...
    emit(AppearanceState.localeChanged(state.info.copyWith(locale: locale)));
  }
  ...
  Future<void> trayIcon({required bool enable}) async {
    ...
    emit(
      AppearanceState.trayIconChanged(state.info.copyWith(trayIcon: enable)),
    );
  }
}

...
var memoizedDateFormatList = memo1(
  (BuiltMap<String, DateFormatEntity> dateFormatMap) => dateFormatList(
    dateFormatMap,
  ),
);
...
List<String> gatewayList(BuiltMap<String, GatewayEntity> gatewayMap) {
  final list = gatewayMap.keys.where(
    (gatewayId) => gatewayMap[gatewayId].isVisible,
  ).toList();

  list.sort(
    (idA, idB) => gatewayMap[idA].sortOrder.compareTo(
      gatewayMap[idB].sortOrder,
    ),
  );
...
var memoizedPaymentTypeList = memo1(
  (BuiltMap<String, PaymentTypeEntity> paymentTypeMap) => paymentTypeList(
    paymentTypeMap,
  ),
);

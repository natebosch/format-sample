...
class PostCreate extends StatefulWidget {
  ...
  PostCreate({this.onDismiss, this.onPosted});
...
class _PostCreateState extends State<PostCreate>
  ...
  Widget build(BuildContext context) {
    Widget w = new Column(
      children: [new _Header(this), new _Body(this), new _Footer(this)],
  ...
  Future onPost() async {
    ...
    try {
      ...
      Map response =
          await lime.api.postMessage(text: _text, compressedImage: image);
  ...
  void initState() {
    super.initState();
    _postingAnimationController = new AnimationController(
      vsync: this,
      duration: const Duration(seconds: 1),
      value: 0.0,
    );
    _postingAnimation = new CurvedAnimation(
      parent: _postingAnimationController,
      curve: Curves.ease,
    );
...
class _Header extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    w = new Stack(
      ...
      children: [w, new Center(child: new Icon(Icons.drag_handle))],
    );

    w = new Container(margin: const EdgeInsets.only(top: 25.0), child: w);

    return new FractionalTranslation(
      translation: new Offset(0.0, -_postCreateState._postingAnimation.value),
      child: w,
    );
  }
}
    ...
    if (image == null) {
      child = new Row(
        ...
        children: [
          ...
          new IconButton(icon: new Icon(Icons.photo), onPressed: pickImage),
        ],
      );
    } else {
      ...
      child = new Container(child: child, margin: const EdgeInsets.all(10.0));
    }

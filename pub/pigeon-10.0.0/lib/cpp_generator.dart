...
class CppHeaderGenerator extends StructuredGenerator<CppOptions> {
      ...
      _writeAccessBlock(indent, _ClassAccess.public, () {
        ...
        for (final Method method in api.methods) {
          ...
          if (method.arguments.isNotEmpty) {
            final Iterable<String> argTypes = method.arguments.map(
              (NamedType arg) {
                final HostDatatype hostType = getFieldHostDatatype(
                  arg,
                  root.classes,
            ...
            final Iterable<String> argNames = method.arguments.map(
  ...
  void _writeClassConstructor(
    ...
    final List<String> paramStrings = params.map((NamedType param) {
      ...
      return '${_hostApiArgumentType(hostDatatype)} ${_makeVariableName(
        param,
      )}';
    }).toList();
  ...
  void _writeCodec(
    ...
    indent.write(
      'class $codeSerializerName : public $_defaultCodecSerializer ',
    );
    indent.addScoped('{', '};', () {
      _writeAccessBlock(indent, _ClassAccess.public, () {
        _writeFunctionDeclaration(
    ...
    _writeFunctionDefinition(
      ...
      body: () {
        ...
        String constructorArgs = nonNullableFields
            .map(
              (_IndexedField param) => getValueExpression(
                param.field,
                'list[${param.index}]',
              ),
            )
            ...
            indent.writeScoped('if (api != nullptr) {', '} else {', () {
              ...
              indent.addScoped('{', '});', () {
                indent.writeScoped('try {', '}', () {
                  ...
                  final String call = 'api->${_makeMethodName(
                    method,
                  )}(${methodArgument.join(', ')})';
                  if (method.isAsynchronous) {
                    indent.format('$call;');
  ...
  void _writeCppSourceClassField(
    ...
    void writeSetter(HostDatatype type) {
      ...
      _writeFunctionDefinition(
        ...
        body: () {
          indent.writeln(
            '$instanceVariableName = ${_fieldValueExpression(
              type,
              setterArgumentName,
            )};',
          );

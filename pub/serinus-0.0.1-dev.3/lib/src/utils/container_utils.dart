...
Controller isController(InstanceMirror controller) {
  int index = controller.type.metadata.indexWhere(
    (element) => element.reflectee is Controller,
  );
  if (index == -1) {
    throw StateError(
      "${controller.type.reflectedType} is in the controllers list of the module but doesn't have the @Controller decorator",
    );
  }
  return controller.type.metadata[index].reflectee;
}

Map<Symbol, MethodMirror> getDecoratedRoutes(
  Map<Symbol, MethodMirror> instanceMembers,
) {
  Map<Symbol, MethodMirror> map = Map<Symbol, MethodMirror>.from(
    instanceMembers,
  );
  map.removeWhere(
    (key, value) =>
        value.metadata.indexWhere((element) => element.reflectee is Route) ==
        -1,
  );
  return map;
}
...
Map<String, dynamic> getRequestParameters(String element, Request request) {
  ...
  for (int i = 0; i < pathSegments.length; i++) {
    if (elementSegments[i].contains(r':') && pathSegments[i].isNotEmpty) {
      pathParameters["param-${elementSegments[i].replaceFirst(':', '')}"] =
          pathSegments[i];
    }
  }
  return pathParameters.isEmpty ? {} : {element: true, ...pathParameters};
}

Module getModule(SerinusModule module) {
  final moduleRef = reflect(module);
  int index = moduleRef.type.metadata.indexWhere(
    (element) => element.reflectee is Module,
  );
  if (index == -1) {
    throw StateError(
      "It seems ${moduleRef.type.reflectedType} doesn't have the @Module decorator",
    );
  }
...
Map<String, dynamic> getParametersValues(
      ...
      if (meta.reflectee is Param || meta.reflectee is Query) {
        if (d.type.reflectedType is! String) {
          switch (d.type.reflectedType) {
            case int:
              routeParameters['$type-$name'] = int.tryParse(
                routeParameters['$type-$name'] ?? '',
              );
              break;
            case double:
              routeParameters['$type-$name'] = double.tryParse(
                routeParameters['$type-$name'] ?? '',
              );
              break;
            default:
        ...
        if (!meta.reflectee.nullable &&
            routeParameters['$type-$name'] == null) {
          throw BadRequestException(
            message: "The $type parameter $name doesn't accept null as value",
          );
        }
      }

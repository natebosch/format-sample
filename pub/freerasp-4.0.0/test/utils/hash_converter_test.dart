  ...
  group('Sha256Converter', () {
    group('toBase64', () {
      group('providing valid value', () {
        ...
        setUp(() {
          validShaValue =
              '00:AA:11:BB:22:CC:33:DD:44:EE:55:FF:66:AA:77:BB:88:CC:99:DD:00:'
              'EE:11:FF:22:AA:33:BB:44:CC:55:DD';
          validBase64Value = 'AKoRuyLMM91E7lX/Zqp3u4jMmd0A7hH/Iqozu0TMVd0=';
        });

        test('standard SHA-256 value', () {
          expect(
            hashConverter.fromSha256toBase64(validShaValue),
      ...
      group('providing invalid value', () {
        ...
        test('unknown delimiter', () {
          final invalidShaValue = validShaValue.replaceAll(':', '|');
          expect(
            () => hashConverter.fromSha256toBase64(invalidShaValue),
            throwsA(isA<FormatException>()),
          );
        });

        test('invalid length', () {
          final invalidLongerShaValue = '${validShaValue}A';
          expect(
            () => hashConverter.fromSha256toBase64(invalidLongerShaValue),
            throwsA(isA<FormatException>()),
          );
          ...
          final invalidShorterShaValue = validShaValue.replaceFirst('F', '');
          expect(
            () => hashConverter.fromSha256toBase64(invalidShorterShaValue),
            throwsA(isA<FormatException>()),
          );
        ...
        test('contains non hex byte', () {
          final invalidHexShaValue = validShaValue.replaceFirst('F', '@');
          expect(
            () => hashConverter.fromSha256toBase64(invalidHexShaValue),
            throwsA(isA<FormatException>()),
          );
        ...
        test('is SHA-1 value', () {
          ...
          expect(
            () => hashConverter.fromSha256toBase64(sha1Value),
            throwsA(isA<FormatException>()),
          );
        });
      });
    ...
    group('fromBase64', () {
      group('providing valid value', () {
        ...
        test('standard base64 value which is sha-256', () {
          expect(
            hashConverter.fromBase64toSha256(validBase64Value),

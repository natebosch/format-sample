...
void main() {
  group('ElGamal', () {
    ...
    test('key generator test', () {
      ...
      engine.init(
        true,
        PublicKeyParameter<ElGamalPublicKey>(keyPair.publicKey),
      );
      expect(
        engine.outputBlockSize,
        2048 ~/ 4,
      ...
      engine.init(
        false,
        PrivateKeyParameter<ElGamalPrivateKey>(keyPair.privateKey),
      );
      expect(
        engine.outputBlockSize,
        (2048 ~/ 8) - 1,
  ...
  group(
    ...
    (() {
      ...
      test('key generator test', () {
        ...
        signer.init(
          true,
          PrivateKeyParameter<DSAPrivateKey>(keyPair.privateKey),
        );
        final signature = signer.generateSignature(message);
  ...
  test(
    'Diffie Hellman key agreement test',
    (() {
      final prime = BigInt.parse(

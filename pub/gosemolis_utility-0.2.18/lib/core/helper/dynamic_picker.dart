...
class DynamicPicker {
  ...
  static Future<List<File>?> showDynamicFilePicker2(BuildContext context,
      {bool multiple = false,
      bool onlyMediaType = false,
      bool showCamera = true}) async {
                ...
                children: <Widget>[
                        ...
                        await PermissionHelper.requestPermissionStorage(
                          onGranted: () async {
                            var files = await _pickFiles(
                                (v) {}, FileType.custom,
                                multiple: multiple, usingAllowedExts: true);
                  ...
                  ListTile(
                      ...
                      onTap: () async {
                        await PermissionHelper.requestPermissionStorage(
                          onGranted: () async {
                            var files = await _pickFiles((v) {}, FileType.media,
                                multiple: multiple);
                            Navigator.pop(context, files);
                          },
                          onDenied: () {
  ...
  static Future<List<File>?> _pickFiles(
      ...
      FileType filetype,
      {bool multiple = false,
      bool usingAllowedExts = false}) async {
    List<File> result = [];
  ...
  static List<String>? _getAllowedExtensions(
      FileType filetype, bool usingAllowedExts) {
    switch (filetype) {
      case FileType.media:
        return null;

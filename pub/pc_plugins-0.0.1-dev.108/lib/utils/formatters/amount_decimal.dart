...
class amountFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    if(newValue.text.isEmpty||Decimal.tryParse(newValue.text)!=null) {
      text = newValue.text; offset = newValue.selection.end;
    }
...
class ssnFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    return newValue.copyWith(text: v, selection: TextSelection.collapsed(offset: s));
  }
}
...
class dollarFmt implements TextInputFormatter{
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    if(n==null) {
      if(newValue.text.contains("..")){
        ...
        return oldValue.copyWith(
            selection:ns..copyWith(baseOffset: ns.baseOffset+1,extentOffset: ns.extentOffset+1));}
      return oldValue;
    }
    else{
      ///if delete, or .
        ...
        for(int i=0;i<oldValue.text.length;i++){
          if(oldValue.text[i]!=newValue.text[i]){
            if(oldValue.text[i]=="."||oldValue.text[i]==","){
              ...
              r = newValue.copyWith(text: formateDollarStr(newx, addSign: false));
            }
            else r = newValue.copyWith(text: n);
            break;
          }
        }
      }
    ...
    return r.copyWith(selection: newValue.selection.copyWith(

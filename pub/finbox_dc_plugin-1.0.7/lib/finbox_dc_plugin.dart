...
class FinBoxDcPlugin {
  static const _platform = const MethodChannel(
    'in.finbox.lending/deviceconnect',
  );
  ...
  static Future<Either<int, String>> createUser(
    ...
    try {
      final response = await _platform.invokeMethod(
        'createUser',
  ...
  static Future setSyncFrequency(int frequency) async {
    try {
      await _platform.invokeListMethod('setSyncFrequency', <String, dynamic>{
        'frequency': frequency,
      });
    } on PlatformException catch (e) {
      print("Error on stop periodic sync ${e.message}");
  ...
  static Future forwardFinBoxNotificationToSDK(
